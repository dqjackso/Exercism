...
(.ingredients + ."optional ingredients" | map(select(.substitute)) | {"ingredients": map(.item)} + {"substitutes": map(.substitute)} | {(.ingredients[0]): (.substitutes[0])} + {(.ingredients[1]): (.substitutes[1])} + {(.ingredients[2]): (.substitutes[2])})
